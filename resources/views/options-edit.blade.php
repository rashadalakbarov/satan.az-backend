...
<div class="row">
    <div class="col-12">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('admin.options.update', $option->id) }}" method="POST" autocomplete="off">
                    @csrf
                    @method('PUT')

                    <div class="mb-3">
                        <label class="form-label" for="default_title">Özəlliyin adı</label>
                        <input type="text" class="form-control @error('default_title') is-invalid @enderror" id="default_title" placeholder="Məs, Məhsulun vəziyyəti" name="default_title" value="{{old('default_title', $option->title)}}">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('default_select') is-invalid @enderror" id="default_select" name="default_select">
                            @foreach($categories as $cat)
                                <optgroup label="{{ $cat->name }}">
                                    @if($cat->children && $cat->children->count())
                                        @foreach($cat->children as $child)
                                            <option value="{{ $child->id }}" @selected($option->category_id == $child->id)>— {{ $child->name }}</option>
                                        @endforeach
                                    @endif
                                </optgroup>
                            @endforeach
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('default_type') is-invalid @enderror" id="default_type" name="default_type">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('default_required') is-invalid @enderror" id="default_required" name="default_required">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('default_activate') is-invalid @enderror" id="default_activate" name="default_activate">

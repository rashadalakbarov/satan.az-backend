...
<div class="row">    
    <div class="col-12">
        <div class="card">
            <h5 class="card-header">Elanların siyahısı ( {{ $elanlar->total() }} ədəd)</h5>
            <div class="card-body">
                <div class="table-responsive text-nowrap mb-4">
                    <table class="table">
                        ...
                        <tbody class="table-border-bottom-0">
                            @forelse($elanlar as $elan)
                                <tr>
                                    <td>{{ $elan->title }}</td>

                                    <td>{{ $elan->user->name }}</td>

                                    <td>                                        
                                        <span class="badge bg-label-{{ 
                                            $elan->activate === 'active' ? 'success' : 
                                            ($elan->activate === 'passive' ? 'primary' : 
                                            ($elan->activate === 'waiting' ? 'warning' : 'info')) 
                                        }} me-1">{{ $elan->activate_text }}</span>
                                    </td>

                                    <td>
                                        <a href="{{route('admin.elanlar.show', $elan->id)}}" class="btn btn-primary edit-city"><i class="fa-solid fa-eye"></i></a>
                                    </td>
                                </tr>
                            @empty
                                <tr>Heç bir elan qeydə alınmayıb.</tr>
                            @endforelse
                        </tbody>                    
                    </table>                
                </div>
                ...
                @if ($elanlar->hasPages())
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            ...
                            {{-- İlk Sayfa --}}
                            <li class="page-item first {{ $elanlar->onFirstPage() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $elanlar->url(1) }}"><i class="tf-icon bx bx-chevrons-left"></i></a>
                            </li>

                            {{-- Önceki Sayfa --}}
                            <li class="page-item prev {{ $elanlar->onFirstPage() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $elanlar->previousPageUrl() ?? '#' }}"><i class="tf-icon bx bx-chevron-left"></i></a>
                            </li>

                            {{-- Sayfa Numaraları --}}
                            @foreach ($elanlar->links()->elements[0] as $page => $url)
                                <li class="page-item {{ $page == $elanlar->currentPage() ? 'active' : '' }}">
                                    <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                                </li>
                            @endforeach

                            {{-- Sonraki Sayfa --}}
                            <li class="page-item next {{ !$elanlar->hasMorePages() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $elanlar->nextPageUrl() ?? '#' }}"><i class="tf-icon bx bx-chevron-right"></i></a>
                            </li>

                            {{-- Son Sayfa --}}
                            <li class="page-item last {{ !$elanlar->hasMorePages() ? 'disabled' : '' }}">
                                <a class="page-link" href="{{ $elanlar->url($elanlar->lastPage()) }}"><i class="tf-icon bx bx-chevrons-right"></i></i></a>
                            </li>
